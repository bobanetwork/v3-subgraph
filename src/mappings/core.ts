...
function loadTickUpdateFeeVarsAndSave(tickId: string, event: ethereum.Event): void {
  let poolAddress = event.address
  let tick = Tick.load(
    poolAddress
      .toHexString()
...
export function handleSwap(event: SwapEvent): void {
  ...
  if (modulo.equals(ZERO_BI)) {
    ...
    loadTickUpdateFeeVarsAndSave(newTick.toString(), event)
  }
  ...
  if (numIters.gt(BigInt.fromI32(100))) {
    ...
    for (let i = firstInitialized; i.le(newTick); i = i.plus(tickSpacing)) {
      loadTickUpdateFeeVarsAndSave(i.toString(), event)
    }
